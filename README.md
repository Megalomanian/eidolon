...
| | Eidolon | Kali (Docker) | Parrot | 自己搭 |
|---|---|---|---|---|
| **AI 副驾驶** | Claude Code 内置 | 无 | 无 | 无 |
| **自然语言渗透** | "帮我审计这个 AD 域" | 手动敲所有命令 | 手动 | 手动 |
| **自动报告** | AI 整合输出 → markdown | 无 | 无 | 自己写脚本 |
| **开箱即用到第一次扫描** | ~30s | 拉完还要 apt install | ~4.7GB 镜像 | 半天 |
| **SOCKS 代理** (`px` 命令) | 一等公民 | 手配 proxychains | 手配 | 手配 |
| **场景分离** | 7 个变体 (按需) | 1 个大镜像 | mono | 无 |
| **非 root 运行** | 是 (UID 1001) | 否 | 否 | 否 |
| **依赖锁定** | 全部锁版本 | apt rolling | rolling | 无 |
